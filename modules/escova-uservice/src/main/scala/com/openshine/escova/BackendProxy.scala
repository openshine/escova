...
object BackendProxy {
  ...
  val maybeHost: Either[String, BackendHost] = {
    ...
    val port = Try(System.getenv("ESCOVA_BACKEND_PORT").toInt).toOption
      .toRight("You must provide a valid int as ESCOVA_BACKEND_PORT")

    host.right.flatMap(host => port.right.map(port => BackendHost(host, port)))
  }
  ...
  def rightRoute(backend: BackendHost)(implicit system: ActorSystem,
                                       materializer: Materializer,
                                       ec: ExecutionContextExecutor): Route = {
    context =>
      ...
      val flow = Http(system)
        .outgoingConnection(backend.host, backend.port)
